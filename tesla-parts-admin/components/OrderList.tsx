...
import { Search, Truck, CreditCard, Pencil, Check, X, Package, ExternalLink, User, MapPin } from 'lucide-react';
...
export const OrderList: React.FC = () => {
  ...
  const filteredAndSortedOrders = React.useMemo(() => {
    const filtered = orders.filter(order => {
      ...
      const productNames = order.items.map(item => (item.product_name || '').toLowerCase()).join(' ');
      
      return customerName.includes(query) || ttn.includes(query) || productNames.includes(query);
    });
                  ...
                  <td className="px-6 py-4">
                    <div className="flex flex-col gap-1">
                      {order.items.slice(0, 2).map((item, idx) => (
                        <div key={idx} className="text-xs text-gray-600 truncate max-w-[200px]">
                          <span className="font-medium">{item.quantity}x</span> {item.product_name || item.product_id}
                        </div>
                      ))}
                  ...
                  <td className="px-6 py-4 max-w-xs">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="text-xs">
                        ...
                        <div className="text-gray-500 truncate max-w-[150px]">{order.delivery_branch}</div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 font-bold">
                    <div className="text-gray-900">{(order.totalUAH ?? 0).toLocaleString()} ₴</div>
                    <div className="text-xs text-gray-400 font-normal">${(order.totalUSD ?? 0).toFixed(2)}</div>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${statusDisplay.className}`}>
                      {statusDisplay.text}
                    </span>
                  </td>
                  <td className="px-6 py-4 text-sm text-gray-700" onClick={(e) => e.stopPropagation()}>
                    {editingTtnOrderId === order.id ? (
                      ...
                      <div className="flex items-center justify-between group">
                        <span className="font-mono text-xs">{order.ttn || '-'}</span>
                        <button
                          onClick={() => {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col">
            ...
            <div className="p-6 border-b border-gray-100 flex items-center justify-between bg-gray-50">
              <div>
                <h3 className="text-xl font-bold text-gray-900">Замовлення #{selectedOrder.id}</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                    ...
                    <div className="text-gray-600 mt-1">{selectedOrder.customer_phone}</div>
                    {selectedOrder.note && (
                ...
                <div className="space-y-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl border border-gray-100 space-y-3">
                    <div className="flex items-start gap-2 text-sm">
                      ...
                      <div>
                        <div className="font-medium">{selectedOrder.delivery_city}</div>
                        <div className="text-gray-500">{selectedOrder.delivery_branch}</div>
                      </div>
                    </div>
                    ...
                    {selectedOrder.ttn && (
                      <div className="pt-2 border-t border-gray-200">
                        <div className="text-[10px] text-gray-400 uppercase font-bold">ТТН</div>
                        <div className="font-mono font-bold text-red-600">{selectedOrder.ttn}</div>
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-gray-50 text-gray-500 text-xs uppercase">
                      <tr>
                        ...
                        <th className="px-4 py-3 text-center font-medium">К-сть</th>
                        ...
                        <tr key={idx} className="hover:bg-gray-50">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              {item.product_image ? (
                                <img src={item.product_image} alt="" className="w-10 h-10 object-cover rounded-md border border-gray-100" />
                              ) : (
                              ...
                              <div>
                                <div className="font-medium text-gray-900">{item.product_name || 'Невідомий товар'}</div>
                                {item.product_detail_number && (
                                  <div className="text-[10px] text-gray-500 font-mono uppercase">{item.product_detail_number}</div>
                                )}
                              </div>
                            </div>
                          </td>
            ...
            <div className="p-6 border-t border-gray-100 bg-gray-50 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-widest ${getStatusDisplay(selectedOrder.status).className}`}>
                  {getStatusDisplay(selectedOrder.status).text}
                </span>
              </div>
              <div className="text-right">
                <div className="text-xs text-gray-500 uppercase font-bold tracking-wider">Разом до сплати</div>
                <div className="text-2xl font-black text-gray-900">
                  {selectedOrder.totalUAH.toLocaleString()} ₴
                </div>
                <div className="text-sm text-gray-400 font-medium">${selectedOrder.totalUSD.toFixed(2)}</div>
              </div>
            </div>
          </div>
        </div>
